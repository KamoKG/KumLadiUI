 ...
 */
app.delete('/removePost/:postID',function(req,res,next){
    if(req.params.postID) //Changed cause it recieves JSON obj with id
    {
        Posts.remove({_id:mongoose.Types.ObjectId(req.params.postID)},function(err,doc){
            if(err)
            {
                return res.status(200).json({
                    status:false,
    ...
    {
        console.log("missing parameters in removePost "+req.params.postID);
        return res.status(200).json({
            "status":false,
            "data":"Missing parameters"
        });
...
var remove=function(obj){
    Posts.find({parent_ID:mongoose.Types.ObjectId(obj)},function(err,posts){
        if(posts.length)
        {
        ...
        Posts.findById(req.params.postID,function(err,post){
            ...
            else if(post)
            {
                ...
                post.save(function(err,post){
                    ...
                    console.log("Saved");
                    Users.findOne({userID:post.student_number},function(err,user){
                        if(err)
                        {
                            return res.status(200).json({
                                status:false
...
app.get('/getUser/:userID', function(req, res, next) {
    ...
    Users.findOne({$or: [{'userID': user}, {'pseodoname': user}]}, function(err, doc) {
        if(err) {
            ...
            if(doc.groupsJoinedTo.length == 0) {
                groupChat.find({}, function(err, obj) {
                    if(err) {
                        console.log("An error occured while searching");
                        return res.status(200).json({});
                    }
                        ...
                        console.log("Searching groups joined");


                        for(var i in obj) { //for each obj
                            for(var j in obj[i].peers) { //for the array
                                if(obj[i].peers[j] == doc.userID) {
                                    doc.groupsJoinedTo.push(obj[i].groupName); //push to user doc
                                }
                            }
                        }

                        doc.save(function(err, docu) {
                            if(err) {
                ...
                /*groupChat.find({}, function(err, obj) {
                    if(err) {
                        ...
                        console.log("Checking if user is in all groups");
                        console.log("doc: " + doc);

                        for(var i in obj) {
                            for(var j in obj[i].peers) {
                                if(obj[i].peers[j] == doc.userID) {
                                    for(var t in doc.groupsJoinedTo) {
                                        if(obj[i].GroupName != doc.groupsJoinedTo[t]) {
                                            doc.groupsJoinedTo.push(obj[i].GroupName);
                                        }
                                    }
                                }
                            }
                        ...
                        doc.save(function(err, docu) {
                            if(err) {
                                ...
                                return res.status(200).json({
                                    "status": false,
                                    "text": "Documen could not be saved",
                                    "data": []
                                });
                            }

                            else {
                                console.log("Document saved - Returning user information");
                                return res.status(200).json({
                                    "status": true,
                                    "text": "Returning user information",
                                    "data": docu
                                });
                            }
                        });
...
var changePoints=function(object,points){
	csStatusS.findOne({userID:object.student_number,courseCode:object.course_code},function(err,cs){
		if(err)
		{
		...
		{
			cs.current_points+=points;
			if(cs.current_points<0)
				cs.current_points=0;
			cs.save(function(err){
				if(err)
					console.log("Error: "+err);
				else
					console.log("Successfully changed the CS status");
			});
		}
		else
		{
			console.log("The CS status for "+post.student_number+" was not found");
		}
	});
};
...
app.get("/getTotalLikes/:userID",function(req,res){
	if(req.params.userID)
	{
		Users.findOne({userID:req.params.userID},function(err,user){
			...
			{
				Votes.find({userID:user.userID},function(err,votes){
					if(err)
					{
						console.log("Error: "+err);
						res.status(200).json({
